...
from VisualTransformerModel import VisualTransformer

OUTPUT_DIR = "ViT-01-lr2"
...
model = VisualTransformer(29).to(device)
...
for e in range(EPOCHS):
    ...
    for _, _, clasVec, imgT in trainLoader:
        imgT = imgT.to(device)
        clasVec = clasVec.to(device)

        preds = model(imgT)

        loss = lossFn(preds, clasVec)
